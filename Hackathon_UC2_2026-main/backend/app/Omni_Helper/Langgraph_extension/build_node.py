...
class NodeBuilder:
    ...
    async def _invoke_agent(
        ...
        """Invoke the agent with the given prompt and return response."""
        try:
            try:
                # Try GAI's LLM first
                if messages:
                    prompt = f"Chat History: ### {messages} ###\n\n{prompt}"
                agent.set_thread_id(thread_id)
                return agent.invoke(prompt), agent.get_thread_id()
            except:
                # Fall back to Graph_GAI's REACT
                return await agent.ainvoke(
                    {
                        "messages": messages[-chat_history_limit:] + [{"role": "user", "content": prompt}],
                        "thread_id": thread_id
                    }
                ), thread_id
    ...
    async def _process(
        ...
        """
        logger.info(f"***{agent_name} Called!***")
        try:
            ...
            response, returned_thread_id = await NodeBuilder._invoke_agent(
                agent, formatted_prompt, messages, chat_history_limit, thread_id
            )
            if not parallel:
...
async def create_nodes_from_config(
    ...
    for agent_name, agent_config in config.items():
        ...
        if not node_config.get("state_key"):
            print(f"Warning: Agent '{agent_name}' is missing a state key. Create a custom function for it!")
